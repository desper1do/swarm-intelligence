...
if __name__ == "__main__":
    ...
    # это и есть основное обучение - отдельного повторного прогона без анимации
    # больше нет, animate_training сама обучает агента на всех N_EPISODES и
    # возвращает историю наград/epsilon для дальнейших графиков.
    # np.random.seed фиксирует и epsilon-greedy случайность агента, и (через
    # seed= в среде) случайность скольжения - без этого при неудачном стечении
    # обстоятельств Q-таблица может ни разу не получить ненулевую награду
    # за все обучение (см. config.py)
    print("Обучение Q-Learning (анимация окна с Q-таблицей и сходимостью)")
    np.random.seed(SEED)
    ql_agent = QLearningAgent(n_states, n_actions)
    ql_rewards, ql_epsilons = animate_training(ql_agent, run_episode_qlearning, N_EPISODES, label="Q-Learning", color="blue")
    ...
    print("\nОбучение SARSA (анимация окна с Q-таблицей и сходимостью)")
    np.random.seed(SEED)
    sarsa_agent = SARSAAgent(n_states, n_actions)
    sarsa_rewards, sarsa_epsilons = animate_training(sarsa_agent, run_episode_sarsa, N_EPISODES, label="SARSA", color="green")
