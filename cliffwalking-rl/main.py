...
from visualize import plot_convergence, plot_epsilon, plot_q_heatmap, plot_policy, plot_trajectory, get_grid_layout, ARROWS
...
def print_policy(q_table, label):
    # быстрый текстовый вывод политики на сетке 4x12 (по одной стрелке на клетку,
    # обрыв помечен C, цель - G)
    rows, cols, start, goal, cliff_cells = get_grid_layout()
    print(f"Политика ({label}):")
    for row in range(rows):
        ...
        for col in range(cols):
            if (row, col) in cliff_cells:
                ...
                state = row * cols + col
                line.append(ARROWS[np.argmax(q_table[state])])
        print(" ".join(line))
...
if __name__ == "__main__":
    ...
    # np.random.seed фиксирует случайность epsilon-greedy агента (сама среда
    # детерминирована, is_slippery=False, так что дополнительной подстройки SEED,
    # как в frozenlake-rl, тут не требуется)
    print("\nОбучение Q-Learning")
    np.random.seed(SEED)
    ...
    print_policy(ql_agent.q_table, "Q-Learning")
    ...
    print_policy(sarsa_agent.q_table, "SARSA")

    # визуализация (п.4 задания): сходимость, epsilon, Q-таблица, политика и
    # траектория для каждого алгоритма отдельно, плюс совместные графики сравнения
    ...
    plot_trajectory(ql_agent.q_table, label="Q-Learning", color="blue", save_path=os.path.join(PLOTS_DIR, "ql_trajectory.png"))
    plot_trajectory(sarsa_agent.q_table, label="SARSA", color="green", save_path=os.path.join(PLOTS_DIR, "sarsa_trajectory.png"))
