"""
Демонстрация работы обученной MAPPO модели.

Пример:
    python eval_demo.py --checkpoint ./checkpoints/mappo/mappo_10gen_terran_ep500.pt --n_episodes 5
"""
    ...
    for ep in range(1, args.n_episodes + 1):
        ...
        for step in range(env.episode_limit):
            ...
            actions, _, actor_hidden = agent.select_actions(
                obs_t, avail_t, hidden_states=actor_hidden, deterministic=True,
            )
...
def main():
    parser = argparse.ArgumentParser(description="Run MAPPO demo with trained model")
